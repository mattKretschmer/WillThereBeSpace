...
def station_output():
	...
	#Set default values if none entered by user.
	...
	dists,inds = find_nearest_station(station_id,station_points)
	...
	end_time,ct = find_time(arrival_t) #Find current time window, time window when arriving at station.
	...
	trip_valuesa = integrate_fluxes(np.zeros(num_of_values),dicta,num_of_values,ct,end_time)
	trip_valuesb = integrate_fluxes(np.zeros(num_of_values),dictb,num_of_values,ct,end_time)
